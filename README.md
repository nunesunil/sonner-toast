...
`Toaster` and `toast` are client-only. Place `<Toaster />` in a Client Component (e.g. `app/providers.tsx` with `"use client"`) and import CSS in the root layout:
...
Styles ship as `sonner-toast/style.css` and use Sonner’s selectors (`[data-sonner-toaster]`, `[data-sonner-toast]`, `.sonner-*`). Override via:
