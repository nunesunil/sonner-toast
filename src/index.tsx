...
type ClassToastType = keyof Pick<
	ToastClassnames,
	"success" | "error" | "info" | "warning" | "loading" | "default"
>;
type IconToastType = keyof Pick<
	ToastIcons,
...
const Toast = (props: ToastProps) => {
	...
	const classNameKey: ClassToastType =
		toastType === "success" ||
		toastType === "error" ||
		toastType === "info" ||
		toastType === "warning" ||
	...
	const duration = toast.duration || durationFromToaster || TOAST_LIFETIME;
	// Height index is used to calculate the offset as it gets updated before the toast array, which means we can calculate the new layout faster.
	const heightIndex = React.useMemo(() => {
		const i = heights.findIndex((height) => height.toastId === toast.id);
		return i === -1 ? 0 : i;
	}, [heights, toast.id]);
	...
	React.useEffect(() => {
		...
		if (toastNode) {
			...
			setHeights((h) => [{ toastId: toast.id, height, position }, ...h]);
			return () =>
	...
	const icon =
		toast.icon ||
		(toastType && toastType in (icons ?? {})
			? icons?.[toastType as IconToastType]
			: undefined) ||
	...
	return (
		<li
			...
			onPointerUp={() => {
				...
				const dragStart = dragStartTime.current;
				const timeTaken = dragStart ? Date.now() - dragStart.getTime() : 0;
		...
		>
			...
			{((toastType || toast.icon || toast.promise) &&
				...
				toastType &&
				toastType in (icons ?? {}) &&
				icons?.[toastType as IconToastType] !== null) ||
			toast.icon ? (
				<div
					data-icon=""
...
function assignOffset(
	...
	[defaultOffset, mobileOffset].forEach((offset, index) => {
		...
		if (typeof offset === "number" || typeof offset === "string") {
			assignAll(offset);
		} else if (typeof offset === "object" && offset) {
			for (const key of OFFSET_KEYS) {
...
const Toaster = React.forwardRef<HTMLElement, ToasterProps>(
	function Toaster(props, ref) {
		...
		const possiblePositions = React.useMemo(() => {
			const positions = filteredToasts
				.map((toast) => toast.position)
				.filter((p): p is NonNullable<typeof p> => p !== undefined);
			return Array.from(new Set([position, ...positions]));
		...
		const listRefs = React.useRef<(HTMLOListElement | null)[]>([]);
							...
							{filteredToasts
								...
								.map((toast, index) => (
									<Toast
										...
										heights={heights.filter((h) => h.position === position)}
